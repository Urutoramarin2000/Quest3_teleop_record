...
def record2dataset(args):
    ...
    for zarr_path in record_dir.glob('*.zarr'):
        ...
        ee_pos = np.array(record['data/ee_pos'])
        ee_rot = np.array(record['data/ee_rot'])
        # joints_pos[np.where(joints_pos[:, 0] < 88), 0] = 0
        # joints_pos[np.where(joints_pos[:, 0] >= 88), 0] = 1
        # joints_pos[:, 0] /= 100
        gripper_pos = record['data/gripper_pos']
        camera_img = record['data/camera_img']
        # memory = record['data/memory']
        resized_imgs =  np.stack([cv2.resize(img, (224,224)) for img in camera_img])
        # episode_data = {'ee_pos': ee_pos, 'ee_rot': ee_rot, 'camera_img': resized_imgs, 'memory': memory, 'gripper_pos': gripper_pos}
        episode_data = {'ee_pos': ee_pos, 'ee_rot': ee_rot, 'camera_img': resized_imgs, 'gripper_pos': gripper_pos}
