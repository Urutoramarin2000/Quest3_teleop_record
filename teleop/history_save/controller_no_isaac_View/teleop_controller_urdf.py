...
from TeleVision_controller_urdf import OpenTeleVision
...
from teleop.utils.IK_solver_pybullet import robot_solver
...
class VuerTeleop:
    def __init__(self, config_file_path):
        ...
        self.tv = OpenTeleVision(self.resolution_cropped, self.img_shm.name, self.num_dof, self.dof_shm.name, toggle_streaming)
...
if __name__ == '__main__':
    ...
    simulator = Sim()
    realsense = RealSenseTools(cameral_serial="218622275838") 


    try:
        while True:
            head_rmat, right_pose, trigger = teleoperator.step()
            # print('right_pose:',right_pose, '------trigger:', trigger)
            new_dof_pos = simulator.step(head_rmat, right_pose, trigger)
            ...
            # print('new_dof', new_dof_pos)
            print(teleoperator.dof_array)
