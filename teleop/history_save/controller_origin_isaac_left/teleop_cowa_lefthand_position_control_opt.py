...
class VuerTeleop:
    def __init__(self, config_file_path):
        ...
        # cowa_retargeting_config = RetargetingConfig.from_dict(cfg['cowa'])
        # self.left_retargeting = left_retargeting_config.build()
        # self.right_retargeting = right_retargeting_config.build()

    def step(self):
        # head_mat, left_wrist_mat, right_wrist_mat, left_hand_mat, right_hand_mat = self.processor.process(self.tv)
        head_mat, left_wrist_mat, right_wrist_mat = self.processor.process(self.tv)
        head_rmat = head_mat[:3, :3]

        left_pose = np.concatenate([left_wrist_mat[:3, 3] + np.array([0.4, -0.1, 0.8]),
...
class Sim:
    def __init__(self,
        ...
        pose.r = gymapi.Quat(0, 0, 0, 1)
        self.cube_handle = self.gym.create_actor(self.env, cube_asset, pose, 'cube', 0)
        color = gymapi.Vec3(1, 0.5, 0.5)
        self.gym.set_rigid_body_color(self.env, self.cube_handle, 0, gymapi.MESH_VISUAL_AND_COLLISION, color)
        cube_idx = self.gym.get_actor_index(self.env, self.cube_handle, gymapi.DOMAIN_SIM)
        ...
        self.cube_root_states = self.root_states[cube_idx]
        ...
        viewer_props.width = 2560  # 设置宽度 (例如 Full HD)
        viewer_props.height = 1440
    ...
    def solve_ik(self, left_wrist, current_l_arm_motor_q = None, current_l_arm_motor_dq = None):
        ...
        self.opti.set_value(self.param_tf_l, left_wrist.homogeneous)
    ...
    def step(self, head_rmat, left_pose, right_pose):
        # enter VR will cause the arm explode, setting to feasible position
        if(left_pose[0]> 1):
            left_pose[:3] = np.array([0.6, 0.0, 0.6])
        if self.print_freq:
            start = time.time()
        # root_states里包含了所有的asset的root位置
        if self.left_compute_flag:
            prev_pose = left_pose[0:7]
            prev_rotation = R.from_quat(left_pose[3:]).as_matrix()
            self.left_compute_flag = False
        else:
            prev_pose = self.left_root_states[0:7].numpy()
        ...
        target_pos = left_pose[:3]
        
        target_rotation = R.from_quat(left_pose[3:]).as_matrix()
        ...
        ### -----gripper
        x_threshold = 0.55  # 右手位姿的 x 坐标阈值
        default_griper_width = 0.02  # 默认夹爪宽度
        griper_width = 0.02
        # 获取右手位置的 x 坐标
        if right_pose is None:
            griper_width = default_griper_width  # 使用默认宽度
        else:  
            if right_pose[0] > x_threshold:
                # 当 x 坐标超过阈值，设置夹爪闭合
...
if __name__ == '__main__':
    ...
    try:
        while True:
            ...
            head_rmat, left_pose, right_pose= teleoperator.step()
            left_img, right_img = simulator.step(head_rmat, left_pose, right_pose)
