...
from teleop.utils.constants_vuer import grd_yup2grd_zup, hand2inspire
from teleop.utils.motion_utils import mat_update, fast_mat_inv
...
class VuerPreprocessor:
    ...
    def process(self, tv):
        ...
        trigger = tv.right_trigger_shared.value
        # print(trigger)
        # homogeneous
        # left_fingers = np.concatenate([tv.left_landmarks.copy().T, np.ones((1, tv.left_landmarks.shape[0]))])
        # right_fingers = np.concatenate([tv.right_landmarks.copy().T, np.ones((1, tv.right_landmarks.shape[0]))])
        ...
        return head_mat, rel_left_wrist_mat, rel_right_wrist_mat, trigger# , rel_left_fingers, rel_right_fingers
